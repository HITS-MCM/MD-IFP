...
# ### Packages required:
    ...
    #     seaborn
    #     MDAnalysis        
    #           Important!   MDAnalysis must be uploaded from the developer version sMDAnalysis==0.19.3.dev0 , see  "https://github.com/MDAnalysis/mdanalysis/wiki/DevelopmentBranch"
...
def IFP(u_mem,sel_ligands,property_list, WB_analysis = True, RE = True,Lipids = []):
    ...
    h = hb.HydrogenBondAnalysis(u_mem, selection1 ='resname '+sel_ligands,selection2=' not resname WAT HOH SOL '+sel_ligands, distance=3.5, angle=110, forcefield='OtherFF')
...
#---------------- extract water bridges between ligand and protein and add to IFP table----------
    if not df_WB.empty:
        ...
        info = True
        """
        for name_col in df_WB.columns.tolist():
            if name_col.find("sele1") >= 0:   
                new_df_WB_columns.append("donor"+name_col[5:])
                if info:
                    print("A wired version of MDAnalysis is used , computationa of water bridges may be incorrect")
                    info = False
            elif  name_col.find("sele2") >= 0:   new_df_WB_columns.append("acceptor"+name_col[5:]) 
            else: new_df_WB_columns.append(name_col)
        df_WB.columns = np.asarray(new_df_WB_columns)
    #    print(df_WB)
        ...
        df_WB_WA = df_WB_ID[df_WB_ID["acceptor_resnm"].isin(["WAT","HOH","SOL"])] # all lines with ligand donor and water acceptor 
        df_WB_WD = df_WB_IA[df_WB_IA["donor_resnm"].isin(["WAT","HOH","SOL"])] # all lines with ligand acceptor and water donor
        t_list = []
        ...
        for t in df_WB.time.unique().tolist():
            ...
            if not df_WB_t.empty:
                df_t = df_WB_t[(df_WB_t.acceptor_resnm != ligand_name) & (df_WB_t.donor_resnm != ligand_name) & ((~df_WB_t.acceptor_resnm.isin(["WAT","HOH","SOL"])) | (~df_WB_t.donor_resnm.isin(["WAT","HOH","SOL"])))] # 
                ...
                # select lines with water from the list of waters interacting with the ligand
                    ...
                    df_P1_a = df_P1[df_P1.donor_resnm.isin(["WAT","HOH","SOL"])]
                    df_P1_d = df_P1[df_P1.acceptor_resnm.isin(["WAT","HOH","SOL"])]
